...
import { useDeleteDepartment } from "@/hooks/meta";
...
const DeleteConfirmModal = ({
  ...
  const { mutate: deleteDepartment, isPending: isDeleting } = useDeleteDepartment(String(selectedDept.id));

  const handleDelete = () => {
    deleteDepartment(undefined, {
      onSuccess: () => {
        toast.success(`"${selectedDept.name}" department permanently deleted`);
        handleClose();
      },
      onError: (err: any) => {
      ...
      >
        ...
        <div className="mb-4">
          ...
          <p className="text-gray-500 font-normal">
            ...
            <span className="font-semibold text-red-500">{selectedDept.name}</span> department? This action cannot be undone.
          </p>
        </div>
          ...
          >
            {isDeleting ? (
              ...
              <>
                ...
                <span>Delete {selectedDept.name}</span>
              </>
            )}
          </button>
        </div>
