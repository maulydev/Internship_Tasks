...
import { useEmptyTrash } from "@/hooks/trash";
import React from "react";
import { CgClose, CgSpinner } from "react-icons/cg";
import { MdDelete } from "react-icons/md";
import { toast } from "react-toastify";

type EmptyTrashConfirmModalProps = {
  showModal: boolean;
  handleClose: () => void;
};

const EmptyTrashConfirmModal = ({
  showModal,
  handleClose,
}: EmptyTrashConfirmModalProps) => {
  const { mutate: emptyTrash, isPending: isDeleting } = useEmptyTrash();

  const handleDelete = () => {
    emptyTrash(undefined, {
      onSuccess: () => {
        toast.success("Trash permanently emptied");
        handleClose();
      },
      onError: (err: any) => {
    ...
    >
      ...
      >
        <button
          ...
          title={
            isDeleting ? "Cannot close while deleting" : "Close modal"
          }
        >
          <CgClose />
        </button>
        ...
        <div className="mb-4">
          ...
          <p className="text-gray-500 font-normal">
            Are you sure you want to permanently empty trash? This action cannot be undone.
          </p>
        </div>
...
export default EmptyTrashConfirmModal;
