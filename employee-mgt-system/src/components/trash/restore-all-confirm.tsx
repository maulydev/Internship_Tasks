"use client";
import { useRestoreAllEmployees } from "@/hooks/trash";
import React from "react";
import { CgClose, CgSpinner } from "react-icons/cg";
import { MdRestore } from "react-icons/md";
import { toast } from "react-toastify";

type RestoreAllConfirmModalProps = {
  showModal: boolean;
  handleClose: () => void;
};

const RestoreAllConfirmModal = ({
  showModal,
  handleClose,
}: RestoreAllConfirmModalProps) => {
  const { mutate: restoreAll, isPending: isRestoring } = useRestoreAllEmployees();

  const handleRestore = () => {
    restoreAll(undefined, {
      onSuccess: () => {
        toast.success("All employees restored successfully");
        handleClose();
      },
      onError: (err: any) => {
      ...
      >
        ...
        <div className="mb-4">
          ...
          <p className="text-gray-500 font-normal">
            Are you sure you want to restore all trashed employees? This action will bring them back into the active list.
          </p>
        </div>
        ...
        <div className="flex justify-end gap-4">
          ...
          <button
            ...
            className="px-4 py-2 bg-green-500 text-white hover:bg-green-600 flex items-center gap-2 cursor-pointer"
          >
            {isRestoring ? (
...
export default RestoreAllConfirmModal;
