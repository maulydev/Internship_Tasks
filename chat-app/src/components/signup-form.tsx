...
const SignUpForm = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (nameError || emailValidation.error || passwordValidation.error) {
      ...
      <div>
        <div className="bg-gray-100 flex items-center pl-4">
          ...
          <input
            ...
            className="px-3 py-4 w-full outline-none select-all:bg-gray-100"
            onChange={handleInputChange}
          />
        </div>
        {error.nameError && (
          <p className="text-xs text-red-500 pt-2">{error.nameError}</p>
        )}
      </div>
